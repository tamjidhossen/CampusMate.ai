...
const HelpRequestSection = () => {
  ...
  const [helpRequests, setHelpRequests] = useState([
    {
      ...
      description: 'Urgent blood donation needed for emergency surgery. Patient is in critical condition.',
      category: 'medical',
      urgency: 'high',
      ...
      responses: [
        {
          ...
          volunteer: {
            ...
            profilePic: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=32&h=32&fit=crop&crop=face',
    ...
    },
    {
      ...
      description: 'Looking for someone to exchange Calculus II textbook (Stewart, 8th edition) for Linear Algebra textbook.',
      category: 'academic',
      urgency: 'low',
    ...
    },
    {
      ...
      description: 'Lost my dorm keys somewhere in the main library. Black keychain with university logo.',
      category: 'general',
      urgency: 'medium',
      ...
      responses: [
        {
          ...
          volunteer: {
            ...
            profilePic: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=32&h=32&fit=crop&crop=face',
    ...
    },
    {
      ...
      description: 'Looking for 2-3 students to form a study group for CHEM 341. Planning to meet twice a week.',
      category: 'academic',
      urgency: 'low',
      ...
      responses: [
        {
          ...
          volunteer: {
            name: 'David Kim',
            department: 'Chemistry',
            profilePic: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=32&h=32&fit=crop&crop=face',
        ...
        },
        {
          ...
          volunteer: {
            ...
            profilePic: 'https://images.unsplash.com/photo-1494790108755-2616b612b212?w=32&h=32&fit=crop&crop=face',
  ...
  const handleSubmitRequest = (e) => {
    ...
    setNewRequest({ title: '', description: '', urgency: 'medium', category: 'general' });
  ...
  const confirmResponse = () => {
    ...
    const newResponse = {
      ...
      volunteer: {
        name: 'Current User', // This would come from authentication
        department: 'Computer Science',
        profilePic: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=32&h=32&fit=crop&crop=face',
        contact: '+1 (555) 000-0000'
      },
      timestamp: new Date()
    };
  ...
  const sortedRequests = [...helpRequests].sort((a, b) => a.timestamp - b.timestamp);
      ...
      <div className="p-6 border-b border-gray-700">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Connect with volunteers for assistance and support</p>
          </div>
          <motion.button
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-lg font-bold text-white">{helpRequests.filter(r => r.status === 'active').length}</div>
            <div className="text-xs text-gray-400">Active Requests</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-lg font-bold text-orange-400">{helpRequests.reduce((sum, r) => sum + r.responses.length, 0)}</div>
            <div className="text-xs text-gray-400">Total Responses</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-3 text-center">
            <div className="text-lg font-bold text-green-400">{helpRequests.filter(r => r.status === 'completed').length}</div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {sortedRequests.map((request) => (
          <motion.div
            ...
            className={`bg-gray-800/80 border rounded-xl p-6 transition-all duration-200 hover:border-gray-600 ${
              ...
              request.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000) ? 'border-orange-500/30 bg-orange-500/5' : 'border-gray-700'
            }`}
          >
            {/* Request Header */}
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center space-x-2">
                {/* Urgency Badge */}
                <div className={`px-2 py-1 rounded-full border text-xs font-medium ${getUrgencyColor(request.urgency)}`}>
            ...
            {request.responses.length > 0 && (
              <div className="mb-4">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-sm text-gray-400">{request.responses.length} volunteer(s) responded</span>
                </div>
                  ...
                  {request.responses.map((response) => (
                    <div key={response.id} className="relative group">
                      ...
                      <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-gray-900 border border-gray-700 rounded-lg p-2 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 whitespace-nowrap">
                        <div className="font-medium">{response.volunteer.name}</div>
                        <div className="text-gray-400">{response.volunteer.department}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {sortedRequests.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm">Be the first to post a help request!</p>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-white">Post Help Request</h2>
                <button
                  onClick={() => setShowNewRequestForm(false)}
      ...
      <AnimatePresence>
        {showResponseModal && selectedRequest && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-white">Respond to Help Request</h2>
                <button
                  onClick={() => setShowResponseModal(false)}
                  className="text-gray-400 hover:text-white p-1"
                >
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="font-medium text-white mb-2">{selectedRequest.title}</h3>
                  <p className="text-gray-400 text-sm">{selectedRequest.description}</p>
                </div>
